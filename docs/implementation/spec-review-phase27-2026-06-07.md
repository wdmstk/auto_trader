...
- worker / order event を route key 基準へ拡張し、同一symbolの複数routeをlive反映可能にした。
- GUI の候補/路線表示でrouteとlimit evidenceを参照しやすくした。
...
- GUI への route-centric schema 統合は後続コミットで検証する。
