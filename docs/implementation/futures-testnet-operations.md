...
補足:
- GUI の `Exchange Position Sync` は repo root の `.env` を自動読込する。`streamlit run` だけで起動する場合でも、`BINANCE_FUTURES_TESTNET_API_KEY` / `BINANCE_FUTURES_TESTNET_API_SECRET` は `.env` に入れておけばよい。
...
- Trading タブの `Exchange Position Sync` で Binance Futures testnet の現在ポジションを直接表示できる
- `exchange` の表示は route 粒度ではなく symbol net 粒度で、local `positions.parquet` との diff を確認する
...
8. Trading タブの `Exchange Position Sync` で、local position と Binance Futures testnet の net position を比較する。
9. 約定後に `position`、`cash`、`PnL` が更新されることを確認する。
10. `STOP` に切り替え、GUI 上で新規注文が止まることを確認する。
11. 画面のスクリーンショットと `order_id` を `docs/implementation/longrun-validation-record-2026-06-01.md` に残す。
...
- Binance REST の exchange sync は local `positions.parquet` を上書きしない。route metadata は REST に無いため、比較と監査のための read-only 表示として扱う。
