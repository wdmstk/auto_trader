...
- `STATISTICAL_GATE_MODE=soft`
  - dry-run / Futures Testnet の検証フェーズ向け
  - `statistical_status=fail` でも route は `selection.trade_routes` に残し、診断ラベルとして扱う
  - Testnetでは execution / position / risk / 運用手順の試行を優先し、統計状態は警告として記録する
- `STATISTICAL_GATE_MODE=hard`
  - Production候補審査・本番運用向け
  - 統計 fail route は `selection.trade_routes` から除外する
  - Productionでは statistical qualification の `pass` を必須とし、fail/missingはfail-closedにする

現状の既定は Testnet試行を想定した `soft`。Production向け成果物を生成する場合は
`STATISTICAL_GATE_MODE=hard` を明示する。
...
- AI-013 の扱い:
  - `trend:SOLUSDT:15m` は `oos_quality` fail のため Production 候補から降格する
  - `route_selection_runtime.production.env` は hard gate の別成果物を参照し、`test-only / statistical-fail` route を本番候補集合に残さない
  - Testnet 側の `route_selection_runtime.env` は soft gate のまま残し、原因分析と運用検証に使う
- AI-014 の扱い:
  - `range:SOLUSDT:30m` は `range_hold16` が autotune 上の最良候補だが、weekly OOS では `pf=0.712`, `expectancy_bps=-15.76`, `period_pnl=-5.03` で失格
  - 近傍の `range_hold0`, `range_hold32`, `range_hold24`, `range_hold8` も `hold16` を上回らず、fold 3 の崩れを解消できていない
  - したがって Production 候補としては据え置かず、Testnet/診断側で再探索する
- AI-015 の扱い:
  - `trend:ADAUSDT:30m` と `trend:BNBUSDT:1h` は `sample_thin` で、closed trades がそれぞれ `16` と `7` に留まる
  - `trend:ADAUSDT:1h` と `trend:ETHUSDT:1h` は `oos_quality` 側でもまだ不安定で、いずれも最終 OOS fold の悪化が残る
  - この段階では再調整を優先せず、route 30 件・strategy 100 件の閾値を満たすまで OOS 蓄積を優先する
  - `trend` 系は watchlist を維持しつつ、retune budget を別の改善候補へ回す
- AI-016 の扱い:
  - 現在の weekly 警告対象 6 route はすべて statistical qualification fail で、portfolio-level の pass 判定に入る母集団がない
  - `candidate_reason` は `core routes=6, probe routes=0, watchlist routes=0` だが、`statistical_reason` は `failed_routes=6` で閉じている
  - したがって portfolio-level DD / 相関の本番判定は、route 単体が pass する候補群が揃ってから別途評価する
  - 先に route 側を pass へ寄せ、そこから同時運用ポートフォリオの risk を積み上げる
- AI-017 の扱い:
- `manifest_vs_weekly_diff` で manifest 採用時と weekly rerun の指標一致を確認できるが、これは tuning と final holdout の完全分離を保証しない
- selection bias 監査は、tuning に未使用の final holdout で再評価することを前提にする
- 現行の weekly rerun は `statistical_fail_diagnostics` まで含めて route ごとの崩れを分解できるので、次は holdout を固定した比較を追加する
- `selection_bias_audit.final_holdout_summary` を見れば、holdout 対比の平均差分を route 全体で追える。
...
- `weekly_revalidation_report.json` は `portfolio_qualification` と `selection_bias_audit` も含み、
  route bundle が同時運用可能かと、final OOS に未通過 route が混入していないかを分けて確認できる。
- `pipeline_summary.json` の `runtime_options` で、Testnet 用 `soft` と Production 用 `hard` を分離して持つ。
- `pipeline_summary.json` の `runtime.strategy_quality_report_path` / `runtime.strategy_quality_report_json_path` で、strategy 品質レポートの正本位置も追える。
- `weekly_revalidation_report.json` 自体は単一 run の選択結果を表すため、`selection.statistical_gate_mode` にはその run で使った mode だけが入る。
- `route_selection_runtime.testnet.env` / `route_selection_runtime.production.env` を見れば、`soft` / `hard` の両方の実運用向け option が追える。
- `portfolio_qualification.strategy_breakdown` を見れば、strategy 別にどの route 組み合わせが通ったかを追える。
- `portfolio_qualification.missing_route_count` / `missing_strategy_count` を見れば、portfolio-level pass までに何件足りないかがわかる。
- `overview.portfolio_qualification_summary.required_route_count` / `required_strategy_count` で、portfolio-level pass の基準値を固定できる。
- `overview.portfolio_qualification_gap_summary.next_route_keys` で、次に見る route 順もまとめて追える。
- `overview.portfolio_qualification_summary.selected_route_keys` / `qualified_route_keys` で、overview から route 実体も追える。
- `overview.portfolio_qualification_summary.reasons` には、portfolio-level fail の直接理由が出る。
- `overview.portfolio_next_action_summary` で、strategy 別に `accumulate_oos` / `drop_or_retune` の実体を読める。
- `overview.portfolio_next_action_summary.*.accumulate_oos_route_keys` / `drop_or_retune_route_keys` で、次に増やす/切る route を直接追える。
- `overview.portfolio_next_action_route_keys` で、portfolio-level の次アクション順を1本で読める。
- `./scripts/portfolio_next_action_report.sh` で、portfolio-level の next action を Markdown で出せる。
- `portfolio_next_action_report.md` は `Qualification` で selected / qualified の route と strategy も併記する。
- `portfolio_next_action_report.md` は `pass_path` で qualified routes / strategies の必要条件も併記する。
- `portfolio_next_action_report.md` は `Pass Gaps` で route / strategy の残差も併記する。
- `portfolio_next_action_report.md` は `Route Coverage` で route-level の next action も併記する。
- `overview.strategy_quality_summary` で、strategy 別の sample thin / oos quality / recommendation を読める。
- `overview.portfolio_strategy_priority_summary` で、strategy 別の優先 route を読める。
- `overview.selection_bias_final_holdout_strategy_summary` で、strategy 別の final holdout 差分を読める。
- `./scripts/strategy_quality_report.sh` で、strategy 品質と holdout 差分を Markdown / JSON の両方で出せる。
- `strategy_quality_report.md` は `accumulate_oos` / `drop_or_retune` の route action も併記する。
- `strategy_quality_report.md` は `Trade Coverage` で route / strategy の closed trades と `Gap To 30` / `Gap To 100` も併記する。
- `strategy_quality_report.md` は `Route Coverage` で accumulate / drop の route 優先順も併記する。
- `strategy_quality_report.json` は Go-Live 正本や自動処理から読む機械可読版になる。
- `./scripts/ai_strategy_progress_report.sh` で、AI-013〜017 の現在地を Markdown / JSON の両方で出せる。
- `ai_strategy_progress_report.md` は AI-013〜017 の現在地と evidence を1枚で読める。
- `ai_strategy_progress_report.json` は AI-013〜017 の現在地を機械可読にする。
- AI-013〜017 の現在地は `ai_strategy_progress_report.md` を正本として読む。
- `ai_strategy_progress_report.md` には strategy 優先 route も併記する。
- `ai_strategy_progress_report.md` には strategy 別の next action も併記する。
- `ai_strategy_progress_report.md` には `Pass Gaps` で portfolio-level の残差も併記する。
- `ai_strategy_progress_report.md` には `Trade Coverage` で OOS closed trades の蓄積進捗と `Gap To 30` / `Gap To 100` も併記する。
- `ai_strategy_progress_report.md` には `Route Coverage` で route-level の next action も併記する。
- `overview.portfolio_strategy_actions` は、strategy 別の bundle pass / review を即時確認するための入口になる。
- `portfolio_strategy_priority_summary` は、strategy 別の優先 route を `accumulate_oos` -> `drop_or_retune` の順で出す。
- `overview.portfolio_strategy_priority_summary` は、運用画面の入口から strategy 優先 route を読める。
- `overview.portfolio_strategy_priority_summary` の `priority_route_keys` は、各 strategy の先頭確認順として使う。
- `route_priority_summary.priority_route_keys` は、AI-013/014 の route 別確認順として使う。
- 現在の週次結果では、`route_priority_summary.priority_route_keys` は
  `trend:ADAUSDT:1h`, `trend:ADAUSDT:30m`, `trend:BNBUSDT:1h`, `trend:ETHUSDT:1h`,
  `range:SOLUSDT:30m`, `trend:SOLUSDT:15m` の順になる。
- `trend` 側は `trend:ADAUSDT:1h` から確認し、`range` 側は `range:SOLUSDT:30m` を先に見る。
- `trend:SOLUSDT:15m` は現週次で `drop_or_retune`、`range:SOLUSDT:30m` も `drop_or_retune` として扱う。
- `overview.selection_bias_final_holdout_summary` には、final holdout の平均差分を集約して出す。
- `overview.selection_bias_final_holdout_strategy_summary` には、strategy 別の final holdout 平均差分を出す。
- `portfolio_risk_audit` は `risk_eval.parquet` を読んで、同時運用時の DD / 相関 / 集中を要約する。
- `selection.dropped_routes` は Production hard gate で除外された route を示す。
- `manifest_vs_weekly_diff.json/md` を生成した run では、`selection_bias_audit.final_holdout_audit` に
  source / weekly の `final_oos` 比較が入る。
- `selection_bias_audit.final_holdout_audit.strategy_deltas` は、final holdout の差分を strategy 別に集約する。
- `selection_bias_audit.final_holdout_summary.strategy_summary` は、strategy 別の平均差分を overview から追うための入口になる。
- `route_quality_audit` は statistical route を `sample_thin` と `oos_quality` に分離して出す。
- `route_quality_audit.route_actions` は `accumulate_oos` / `drop_or_retune` の推奨を返す。
- `route_quality_audit.strategy_counts` は strategy 別の sample_thin / oos_quality 件数を返す。
- `route_quality_summary` は recommendation / category 別の件数サマリを返す。
- `strategy_quality_summary` は `trend` / `range` ごとの優先度を返す。
...
- `weekly_autotune_pipeline.sh` は `portfolio_next_action_report.md` も同じ `weekly_revalidation` 配下へ自動生成する。
- `weekly_autotune_pipeline.sh` は `strategy_quality_report.md` も同じ `weekly_revalidation` 配下へ自動生成する。
- `weekly_autotune_pipeline.sh` は `strategy_quality_report.json` も同じ `weekly_revalidation` 配下へ自動生成する。
- `weekly_autotune_pipeline.sh` は `ai_strategy_progress_report.md` も同じ `weekly_revalidation` 配下へ自動生成する。
- `weekly_autotune_pipeline.sh` は `ai_strategy_progress_report.json` も同じ `weekly_revalidation` 配下へ自動生成する。
...
## route-centric longrun / testnet 証跡

AI-012 では、週次正本の `run_id` を起点に longrun と Futures Testnet の証跡を追えることを重視する。

標準の証跡入口:

- `data/validation/weekly_autotune/pipeline_summary.json`
- `data/validation/weekly_autotune/weekly_revalidation/weekly_revalidation_report.json`
- `data/validation/weekly_autotune/weekly_revalidation/statistical_fail_diagnostics.json`
- `data/validation/weekly_autotune/weekly_revalidation/manifest_vs_weekly_diff.json`
...
- `docs/implementation/longrun-validation-record-YYYY-MM-DD.md`
...
`PIPELINE_RUN_ID` を渡した場合は、longrun/testnet 側の出力にも同じ `run_id` が入り、週次再検証成果物と証跡を同一 run として追跡できる。
...
- `portfolio_qualification.status=pass` は、少なくとも 2 route かつ 2 strategy が final OOS を通過し、bundle としても通ることを示す。
- `portfolio_risk_audit.status=warn` は、risk_eval 側で DD / correlated exposure / vol weighted exposure / risk contribution のいずれかが過大であることを示す。
- `selection_bias_audit.status=warn` は、選定 route に final OOS 未通過の route が混ざっていることを示し、tuning と final holdout の分離不足を疑う。
- `route_quality_audit` で `sample_thin_route_keys` が残る場合は、まず OOS 蓄積を優先する。
- `route_quality_audit` で `drop_or_retune` が残る場合は、Production 候補から外すか再調整する。
- `route_quality_summary.recommendations` を見れば、OOS 蓄積と切り戻しの件数を一目で確認できる。
- `strategy_quality_summary.trend` は sample_thin が多いなら OOS 蓄積、`range` は oos_quality があれば降格優先。
