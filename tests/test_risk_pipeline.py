...
def test_run_risk_pipeline_derives_vol_weighted_metrics(tmp_path: Path) -> None:
    base = datetime(2026, 1, 1, tzinfo=UTC)
    rows = [
        {
            "timestamp": base,
    ...
    out = run_risk_pipeline(
        ...
        config=RiskConfig(
            ...
            max_vol_weighted_exposure_pct=50.0,
        ),
    )
    assert "vol_weighted_exposure_pct" in out.columns
