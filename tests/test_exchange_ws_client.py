...
def test_parse_message_from_wrapped_stream_payload() -> None:
    raw = (
        ...
        '"i":12345,"c":"cid_001","s":"BTCUSDT","S":"BUY","X":"FILLED","z":"0.15","ap":"64001.5"}}'
    )
    ...
    assert abs(ev.avg_fill_price - 64001.5) < 1e-9
...
def test_parse_message_from_futures_order_trade_update() -> None:
    raw = '{"e":"ORDER_TRADE_UPDATE","E":1704067200000,' '"o":{"i":12345,"c":"cid_fut_001","s":"BTCUSDT","S":"SELL","X":"EXPIRED","z":"0.25"}}'
    client = BinanceWsExecutionClient()
    ev = client.parse_message(raw)
    assert ev is not None
    assert ev.order_id == "12345"
