...
OUT_PATH="${OUT_PATH:-data/validation/weekly_autotune/weekly_revalidation/ai_strategy_progress_report.md}"
...
route_priority = overview.get("route_priority_summary", {})
...
portfolio = report.get("portfolio_qualification", {})
if not isinstance(portfolio, dict):
    portfolio = {}
qualification_summary = overview.get("portfolio_qualification_summary", {})
if not isinstance(qualification_summary, dict):
    qualification_summary = {}

gate_split_summary = {
    "soft": {
        "purpose": "Testnet / analysis",
        "current_role": "statistical fail route を残して原因分析と運用検証に使う",
    },
    "hard": {
        "purpose": "Production / fail-closed",
        "current_role": "statistical pass route のみを本番候補に残す",
    },
}
...
entries = {
    "AI-013": {
        "status": "drop_or_retune",
        "summary": "trend:SOLUSDT:15m は Production 候補から外す前提で扱う",
        "evidence": [
            "strategy_quality_summary.trend.recommendation=drop_or_retune",
            "route_priority_summary.priority_route_keys に trend:SOLUSDT:15m を含む",
        ],
    },
    "AI-014": {
        "status": "drop_or_retune",
        "summary": "range:SOLUSDT:30m は fold 3 崩れが残るため再探索対象",
        "evidence": [
            "strategy_quality_summary.range.recommendation=drop_or_retune",
            "route_priority_summary.priority_route_keys に range:SOLUSDT:30m を含む",
        ],
    },
    "AI-015": {
        "status": "sample_thin",
        "summary": "ADA/BNB/ETH は OOS 蓄積優先",
        "evidence": [
            "portfolio_next_action_summary.trend.accumulate_oos_route_keys",
            "strategy_quality_summary.trend.sample_thin_count=4",
        ],
    },
    "AI-016": {
        "status": "fail",
        "summary": "portfolio-level pass には未到達",
        "evidence": [
            f"portfolio_qualification.status={portfolio.get('status', 'unknown')}",
            f"missing_route_count={portfolio.get('missing_route_count', 0)}",
            f"missing_strategy_count={portfolio.get('missing_strategy_count', 0)}",
        ],
    },
    "AI-017": {
        "status": "visible",
        "summary": "holdout 比較と strategy summary は可視化済み",
        "evidence": [
            "selection_bias_audit.final_holdout_summary.strategy_summary",
...
lines: list[str] = []
lines.append("# AI-013-017 Progress Report")
lines.append("")
lines.append(f"- generated_at: {payload['generated_at']}")
lines.append(f"- report: {report_path}")
lines.append(f"- portfolio_status: {payload['portfolio_status']}")
lines.append("")
lines.append("| ID | Status | Summary | Evidence |")
lines.append("|---|---|---|---|")
for ai_id in ["AI-013", "AI-014", "AI-015", "AI-016", "AI-017"]:
    item = entries[ai_id]
    lines.append(
        "| {ai_id} | {status} | {summary} | {evidence} |".format(
            ai_id=ai_id,
            status=item["status"],
...
lines.append("")
lines.append("## Gate Split")
lines.append("| Mode | Purpose | Current Role |")
lines.append("|---|---|---|")
for mode, item in gate_split_summary.items():
    if not isinstance(item, dict):
        continue
    lines.append(
        "| {mode} | {purpose} | {current_role} |".format(
            mode=mode,
...
lines.append("| Strategy | Route Count | Delta PF | Delta Expectancy (bps) | Delta Period PnL | Delta Max DD | Delta Closed Trades |")
lines.append("|---|---:|---:|---:|---:|---:|---:|")
...
required_route_count = int(
    qualification_summary.get(
        "required_route_count", portfolio.get("required_route_count", 0)
    )
    or 0
)
required_strategy_count = int(
    qualification_summary.get(
        "required_strategy_count", portfolio.get("required_strategy_count", 0)
    )
    or 0
)
qualified_route_count = int(
    qualification_summary.get(
        "qualified_route_count", portfolio.get("qualified_route_count", 0)
    )
    or 0
)
qualified_strategy_count = int(
    qualification_summary.get(
        "qualified_strategy_count", portfolio.get("qualified_strategy_count", 0)
    )
    or 0
)
...
lines.append(f"- selected_route_count: {int(portfolio.get('selected_route_count', 0) or 0)}")
lines.append(f"- selected_strategy_count: {int(portfolio.get('selected_strategy_count', 0) or 0)}")
