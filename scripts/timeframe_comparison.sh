...
RANGE_SR_SUPPORT_DISTANCE_MAX="${RANGE_SR_SUPPORT_DISTANCE_MAX:-1.5}"
RANGE_SR_MIN_LEVEL_STRENGTH="${RANGE_SR_MIN_LEVEL_STRENGTH:-2}"
  ...
  for strategy in ${STRATEGIES//,/ }; do
    if [[ "$strategy" == "range" ]]; then
      "$PYTHON_BIN" -m auto_trader.strategy \
        ...
        --range-sr-support-distance-max "$RANGE_SR_SUPPORT_DISTANCE_MAX" \
        --range-sr-min-level-strength "$RANGE_SR_MIN_LEVEL_STRENGTH" \
        --range-sr-resistance-exit-atr "$RANGE_SR_RESISTANCE_EXIT_ATR" \
        --range-w-sr-proximity "$RANGE_W_SR_PROXIMITY" \
        --range-w-sr-strength "$RANGE_W_SR_STRENGTH" \
